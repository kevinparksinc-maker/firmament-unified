...
import { AtlasAspectAudit, AtlasPointRelations } from "@/components/AtlasAspectAudit";
...
function dialogueChartPayload(chart: ReturnType<typeof calculateZeteticChart>) {
  return {
    ...
    points: chart.points.map(({ name, kind, longitude, sign, degree, house, rightAscension, declination, azimuth, altitude, nakshatra, fixedStars, dignity, combustion, motion }) => ({ name, kind, longitude, sign, degree, house, rightAscension, declination, azimuth, altitude, nakshatra, fixedStars, dignity, combustion, motion })),
    planetaryWars: chart.planetaryWars,
    aspectScan: chart.aspectScan,
  };
}
...
function combustionLabel(point: ZeteticPoint) {
  ...
  if (point.combustion.isKazimi) return `Kazimi · ${distance} ≤ 0.5°`;
...
function DignityEvidence({ point }: { point: ZeteticPoint }) {
  return <div style={{ borderTop:"1px solid rgba(26,44,49,.16)", marginTop:10, paddingTop:10, fontSize:12, color:"#334c50", lineHeight:1.45 }}>
    ...
    <strong style={{ color:"#1c3c41" }}>Solar condition:</strong> {point.combustion.applicable ? <>{point.combustion.isKazimi ? "Kazimi" : point.combustion.isCombust ? "Combust" : "Not combust"} · Sun distance {degree(point.combustion.angularDistance ?? 0)} · combustion threshold {degree(point.combustion.threshold)}</> : "Not applicable"}<br />
    <strong style={{ color:"#1c3c41" }}>Longitudinal motion:</strong> {point.motion.applicable ? <>{point.motion.isRetrograde ? "Retrograde" : "Direct"} · {point.motion.speedDegreesPerDay?.toFixed(3)}°/day</> : "Not applicable"}<br />
    <span style={{ color:"#58716f" }}>{point.combustion.rule} {point.motion.rule}</span>
  </div>;
}
