...
export default function ZeteticAtlas() {
  ...
  const reset = () => { setInput(SAMPLE); setChart(calculateZeteticChart(SAMPLE)); setSelectedKey("sun"); setMode("gleason"); setError(""); };
  ...
  return <main className="zetetic-atlas-page">
    ...
    <div className="zetetic-shell">
      ...
      <div className="zetetic-layout">
        ...
        <section className="atlas-stage"><div className="atlas-stage-heading"><div><p className="atlas-eyebrow">{mode === "gleason" ? "Gleason celestial projection" : "Local topocentric compass sky"}</p><h1>{chart.input.location || "Untitled coordinate"}</h1><p>Live Astronomy Engine ephemeris · direct Zetetic chart frame</p></div><div className="atlas-controls"><button type="button" className={mode === "gleason" ? "is-active" : ""} onClick={() => setMode("gleason")}>GLEASON MAP</button><button type="button" className={mode === "compass" ? "is-active" : ""} onClick={() => setMode("compass")}>COMPASS SKY</button><button type="button" className={showAudit ? "is-active" : ""} onClick={() => setShowAudit((value) => !value)}><Eye size={12} style={{ verticalAlign:"-2px", marginRight:5 }} />AUDIT</button></div></div>
          ...
          {showAudit && <section className="atlas-audit"><h2>Audit mode · live calculation trace</h2><div className="atlas-audit-grid"><section><dl><dt>LOCAL INPUT</dt><dd>{chart.input.birthDate} {chart.input.birthTime} · {chart.input.timezone}</dd><dt>UTC INSTANT</dt><dd>{chart.utcDate.toISOString().replace("T", " ").slice(0, 19)}Z</dd><dt>LONGITUDE OFFSET</dt><dd>{degree(chart.input.longitude)}</dd></dl></section><section><dl><dt>UTC DEGREES</dt><dd>{degree(chart.utcDegrees)}</dd><dt>MC = UTC° + LONGITUDE</dt><dd>{degree(chart.midheaven)} · {chart.points.find((point) => point.key === "midheaven")?.sign}</dd><dt>ASC = MC + 90°</dt><dd>{degree(chart.ascendant)} · {chart.points.find((point) => point.key === "ascendant")?.sign}</dd></dl></section></div><div className="atlas-scroll"><table><thead><tr><th>Point</th><th>Tropical longitude</th><th>House</th><th>RA</th><th>Dec</th><th>Azimuth</th><th>Altitude</th><th>Gleason X/Y</th><th>Compass X/Y</th></tr></thead><tbody>{chart.points.map((point) => { const gleason = gleasonPoint(point.rightAscension, point.declination); const compass = compassPoint(point.azimuth, point.altitude); return <tr key={point.key}><td>{point.name}</td><td>{point.signSymbol} {point.sign} {degree(point.degree)}</td><td>H{point.house}</td><td>{point.rightAscension.toFixed(2)}h</td><td>{degree(point.declination)}</td><td>{degree(point.azimuth)}</td><td>{degree(point.altitude)}</td><td>{gleason.x.toFixed(1)} / {gleason.y.toFixed(1)}</td><td>{compass.x.toFixed(1)} / {compass.y.toFixed(1)}{compass.visible ? "" : " · below horizon"}</td></tr>; })}</tbody></table></div><p className="atlas-copy" style={{ margin:"12px 0 0" }}>Tropical ecliptic longitude establishes zodiac and Equal Houses. RA/declination is independently projected onto the Gleason map. Topocentric azimuth/altitude is used only for the separate compass display.</p></section>}
        </section>
        <aside className="zetetic-rail zetetic-detail"><section><p className="atlas-eyebrow detail-eyebrow">Selected placement</p><h2 className="atlas-title">{selected.name}</h2><p className="atlas-copy"><strong>{selected.signSymbol} {selected.sign} {degree(selected.degree)}</strong> · House {selected.house}</p><div className="atlas-stat-grid"><div className="atlas-stat"><b>{degree(selected.longitude)}</b><span>Tropical long.</span></div>{mode === "gleason" ? <div className="atlas-stat"><b>{degree(selected.declination)}</b><span>Declination</span></div> : <><div className="atlas-stat"><b>{degree(selected.azimuth)}</b><span>Azimuth</span></div><div className="atlas-stat"><b>{degree(selected.altitude)}</b><span>Altitude</span></div></>}</div><p className="atlas-house"><strong>House {selected.house}:</strong> {chart.houses[selected.house - 1]?.startLabel} → {chart.houses[selected.house - 1]?.endLabel}</p></section><section><p className="atlas-eyebrow detail-eyebrow" style={{ marginTop:22 }}>Reference points</p><div className="atlas-point-list">{chart.points.filter((point) => point.kind !== "planet").map((point) => <button className={`atlas-point-row ${point.key === selected.key ? "is-active" : ""}`} key={point.key} type="button" onClick={() => setSelectedKey(point.key)}><i className="atlas-dot" style={{ background:point.color }} /><span>{point.name}</span><span>{point.signSymbol} {degree(point.degree)}</span></button>)}</div></section><section><p className="atlas-eyebrow detail-eyebrow" style={{ marginTop:22 }}>Planet index</p><div className="atlas-point-list">{chart.points.filter((point) => point.kind === "planet").map((point) => <button className={`atlas-point-row ${point.key === selected.key ? "is-active" : ""}`} key={point.key} type="button" onClick={() => setSelectedKey(point.key)}><i className="atlas-dot" style={{ background:point.color }} /><span>{point.name}</span><span>{point.signSymbol} {degree(point.degree)}</span></button>)}</div></section></aside>
      </div>
    </div>
  </main>;
}
