...
This document makes the coordinate systems in `god-agent-family-flow-v1` inspectable across a historical event cohort. Its aim is to distinguish **changing numerical data** from a repeated final polarity category. It is not a ruleset amendment and does not add a topocentric score, alter the frozen God-axis sectors, or turn the output into a wagering recommendation.
...
| Topocentric observation | Same UTC and stadium latitude/longitude, with altitude set to zero unless explicitly provided | Astronomy Engine local observer → topocentric equatorial RA/declination → altitude/azimuth via `Horizon` | **No — audit only** |

The God View deliberately has no observer input. Its result changes with the event instant but not with a stadium coordinate. The Agent View is local because its house cusps depend on event place, but its existing planet-longitude helper is geocentric ecliptic-of-date. The topocentric altitude/azimuth values to be displayed in this audit are an additional observational coordinate set, not a replacement for either one.
...
The frozen August 22–26 NPB cohort returned 20 DSC convergences, not 20 identical skies. It is a five-day, time-clustered cohort; every God-side result is a sector-count classification at its own UTC instant, and every Agent-side result is a local Placidus-house-family classification at its own start time and venue. The following per-game audit will expose the raw variation and quantify whether a proposed topocentric observation is materially distinct, without scoring it.

## Measured NPB Variation

The enriched audit retained all 20 standard-orientation NPB calculations from August 22–26. Every record was `dsc-convergence`, but the raw inputs varied materially:
...
The wide cusp ranges reflect different local event times and stadium longitudes/latitudes, with circular wraparound in some cusp columns. The topocentric ranges also confirm that the planets were not in one identical local sky configuration. The repeated result is therefore a **classification persistence** finding: the current binary God sector and ASC/DSC family counts did not cross their decision boundaries during this short window. It is not evidence that the ephemeris returned identical positions or that KP, Lots, or Nakshatra calculations failed.
...
A normal browser submission of the frozen Target Field record rendered the new **Topocentric compass observation** section with the `unscored observation` label, alongside the existing God View, Agent View, Family Flow, and secondary-aspect audit. The same submission retained God View `1 ASC / 1 DSC` (`tie`), Agent View `2 ASC-family / 5 DSC-family` (`dsc`), synthesis `neutral`, and public outcome `no-call`. The refreshed page showed no application exception in the rendered result path.
