...
The first 20 eligible games, in the declared reverse-chronological August 22–26 sequence, were evaluated under frozen standard-orientation `god-agent-family-flow-v1`. The archived market underdog won **11 of 20** games (55.0%; Wilson 95% interval 34.2%–74.2%). The two-sided archived prices imply 8.70 aggregate underdog wins after normalizing their quoted overround (mean no-vig underdog probability 43.5%), so the observed count was 2.30 wins above that descriptive price baseline. The model returned **zero no-calls** and 20 directional context results, all `dsc-convergence` / Side B–challenger context. Therefore the no-call subgroup contains 0 games, its underdog rate is undefined, and this cohort cannot support or refute the proposed “no-call implies underdog” hypothesis. The official outcomes were joined only after each chart calculation, as the saved evaluation audit records.
