...
| Cluster / Layer Vote | **Tyson / Side A — separately reported legacy result** | The current legacy compatibility route was reproduced from a structured **topocentric Tokyo Dome** chart constructed at the frozen calibration instant. It uses declared Equal Houses from the exact Ascendant: Territorial chose A (42.56 vs −18.38); KP Stellar chose A (25.00 vs 3.50); the visible layer-vote result was A 5, B 2, with 7 ties. | Did not match the observed Side B win. This manual-chart compatibility result remains excluded from the strict-method ledger. |
...
After the strict-method review, the legacy Cluster compatibility path was also rerun from the same declared calibration instant. The event chart was first produced by `ephemeris.calculate` using the **exact Tokyo Dome observer** (35.7056, 139.7519), then its pre-event placements, exact ecliptic longitudes, source labels, Ascendant, and Equal House cusps were supplied to `sportsHorary.askWithChart`. Neither the observed winner nor any post-fight fact entered this request.
...
The first correction preserves each raw longitude, explicitly reassigns every planet from the exact Ascendant before Territorial Control, KP, Lots, or any Cluster vote runs, and displays both values in the UI. The Whole Sign column remains available as a labelled ephemeris reference only; it is not a scored Cluster input.

The next audit found that the physical-planet ecliptic values themselves had been sourced from an Earth-center vector even while the Ascendant and horizon panel used a stadium-local observer. The current route now uses Astronomy Engine's observer-relative vector for each physical planet and labels it `topocentric-apparent-ecliptic`. Rahu and Ketu remain clearly labelled `mean-node-ecliptic` mathematical points. A further route defect reduced these values to rounded text before the Cluster scorer received them; that text round-trip has been retired. The current public Cluster path accepts only exact structured coordinates, recomputes the Equal House map server-side, and refuses text-only scoring. The city selector is now a coordinate prefill only; its editable stadium latitude and longitude are required and remain visible with the generated chart.
...
| Topocentric observer | Tokyo Dome: 35.7056, 139.7519 | Same observer; every ecliptic point and Ascendant rotated 180° only for audit |
| Raw point margin, diagnostic only | +88.44 | +67.96 |
| Territorial Control layer vote | A 42.56; B −18.38 | A 31.40; B −26.06 |
| KP Stellar layer vote | A 25.00; B 3.50 | A 27.00; B 4.50 |
| Eligible layer choices | A 5; B 2; ties 7 | A 4; B 3; ties 7 |
| Side B layers in the standard result | Lunar Flow; Chart-wide Aspects | Chart-wide Aspects; Translation of Light |
| Detailed Territorial report (separate additive scale) | A +18.75; B −2.56 | A −0.38; B −10.69 |

This current topocentric, exact-coordinate reproduction still does **not select Douglas** for this fixed record. It is less lopsided than the superseded mixed-house run, changing from 6–1 to 5–2 eligible layer choices, but it remains a Tyson/Side A output. The repository history contains no saved Tyson–Douglas payload or committed result showing Territorial or KP selecting Douglas. A remembered Douglas-side output therefore cannot be treated as evidence until its exact timestamp, manual placements, Ascendant/cusps, orientation, code revision, and full layer record are supplied and rerun.
